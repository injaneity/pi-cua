...
threads outside Git repositories use the sandbox user's persistent home directory. placement, tools, shell commands, reload, `/new`, and `/fork` work normally, but Pi does not copy or synchronize the controller's arbitrary local directory. switching targets retires the tool host without deleting files in the sandbox home.
...
placement adds a stable operating-system instruction and a logical `workspace root` or `sandbox home` cwd to the model prompt, but no sandbox name, physical path, or model-visible conversation entry. replacement sessions on the same target OS therefore keep the same prompt prefix for provider cache hits.
