...
2. one ssh preflight checks machine health and the mutable guest-bundle digest; Git-backed threads also check free disk and repository-cache availability;
...
each sandbox execution derives an opaque id from the Pi thread. Git-backed execution uses it for the isolated workspace; non-Git execution uses it for a persistent directory under the sandbox user profile. `/new` and `/fork` transfer that execution to the replacement session, while the previous session records local placement. failed Git destination setup removes its incomplete workspace before returning the error.
...
on Windows, OpenSSH remains in Session 0 and forwards its authenticated channel directly to the loopback-only broker. the logged-in user's scheduled task runs that broker directly and keeps execution-directory-scoped Pi hosts in Session 1, so GUI tools and shell commands share the interactive desktop without another remote process. disconnects detach from the host; reconnects reuse it until its execution is retired or its tool configuration changes.
