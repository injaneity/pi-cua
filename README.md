...
`cua_sandbox`, `enter_environment`, `report_papercut`, and `web_search` stay on the controller. Web search uses controller credentials, not guest workspace state; its extension and API key are not staged for remote execution. Other registered tools are proxied by name, except for explicit controller file routes: read-only Pi clipboard images and Exa's temporary `pi-exa-search-*/results.txt` full-output files, plus papercut ledger reads/edits/writes. Ordinary workspace file operations stay remote. sandbox activation fails if the remote pi sdk host does not expose a required tool; calls never fall back to local execution. tools registered after activation are blocked until `/reload` rebuilds the routed tool set, and the active sandbox cannot be deleted until the session returns to local execution.
...
`read`, `find`, and `grep` (including fff's prefixed names) translate known controller resource paths into the copied guest runtime. Reads identify the actual guest path so supporting scripts can use it. Ordinary workspace paths are untouched, missing remote files never fall back locally, and mapped configuration writes are rejected. To change configuration, edit it on the controller and reconnect or reload; a live connection is a snapshot, not continuous synchronization. Config/resource changes participate in the runtime digest. Known resource paths from earlier generations under the same guest runtime root are translated to the active snapshot before dispatch, with the actual path shown in the result. Unknown paths are not guessed or retried locally. Runtime snapshots consume disk until explicitly cleaned up; only remove generations after confirming that no session uses them.

Papercuts are live controller state, not a runtime snapshot. `report_papercut` and `read`/`edit`/`write` of `~/.pi/agent/papercuts/<project-key>/papercuts.md` use the same controller ledger across sandbox switches, including reports added after entry. The configured Pi agent directory is honored. Results identify their controller location. Edits and writes use the SDK's normal local file tools and mutation queue; this does not make other controller files writable from a sandbox. Guest shell commands and search tools still operate on guest files. This is a narrow path rule, not missing-file fallback. Project keys still follow the papercuts extension's controller working-directory identity; unrelated checkouts are not merged.

Tool activation verifies the remote provider, not just its name: builtin search cannot silently stand in for fff. The controller proxy must also win Pi's extension registration precedence. Load pi-cua before competing tool packages. For an auto-discovered local installation, place `"./extensions/cua-sandbox"` first in the global `settings.json` packages array, then `/reload`. This uses normal Pi configuration, not a modified SDK. Package order does not necessarily reorder standalone extensions; controller-only web search does not require a proxy or compete for that precedence.
...
3. one content-addressed runtime is staged under `~/.cua-pi/runtimes/<digest>`, installs exact npm versions and Git commits with production dependencies but without private copies of Pi peer packages, and is published only after package reconciliation succeeds; generations coexist instead of rewriting shared Pi settings; installations do not delete older generations because age does not establish that a runtime is unused;
...
Git-backed target changes use one workspace model: Pi records local and sandbox Git trees at sandbox entry, computes one accumulated binary tree diff from the original commit to the source's final tree, verifies the destination tree, and applies that diff. Incoming changes are staged in a freshly prepared sandbox so tracked files remain represented even when ignore rules match them, and executable modes survive platform changes. Entry exchanges content-addressed Git objects instead of a cumulative binary patch: it checks the destination's objects, can refresh its repository cache from origin, and transfers only missing objects through the controller. The destination verifies the complete working tree before activation. The original local merge baseline is retained and pinned separately, including when the source already holds its only copy. Snapshots copy the existing index into a temporary index rather than rebuilding tracking from `HEAD`; the original index is not changed by capture. Local return explicitly tracks newly imported ignored files while preserving existing partial staging. Untracked ignored build outputs are still excluded. after a successful move to local or another sandbox, it removes the source workspace, including ignored build outputs. shutdown preserves the workspace and placement. `/new` and `/fork` inherit the sandbox choice and copy the parent's current Git working state into a separate workspace, excluding ignored outputs. a fork does not restore historical filesystem contents. non-Git children start with a separate empty execution directory. startup, reload, resume, explicit reselection, and bridge reconnection resolve the stable sandbox name to its current network endpoint. the normal resume path opens the exact saved runtime and workspace directly; if that open fails before an operation runs, activation performs the full validation and reconstruction path once. non-conflicting local and sandbox edits are merged during sync. a patch conflict or cleanup failure is reported explicitly without modifying the local files, and a failed sync retains the remote workspace rather than deleting the only copy of changes.
...
Entry records `cua-backend-timing` and `cua-entry-timing` session entries. Backend durations are milliseconds, except `workspace.object_bytes`, which counts the framed pack payload. The entry record separates preparation, tool-host connection, and source cleanup. These records do not contain credentials or file contents.

SSH multiplexing is scoped to one backend request, in a private temporary directory. Each request establishes fresh host-key-verified connections; connections are closed on completion and are not reused across requests or generations. Identity checks and workspace locks remain in place.

`node benchmark-entry.mjs <fixture.json>` measures a full macOS-to-Linux entry in a fresh controller SDK session, including tool-host connection and source cleanup. It clones an isolated source from the supplied template, creates a fresh destination, verifies the expected tree and a transferred fixture file, and removes only its own workspaces. Model networking is disabled. A fixture supplies `mac_template`, `mac_address`, `mac_generation`, `tree`, and the normal workspace `state`; the template must retain the expected tree and original local baseline. The active user workspace must not be used as a benchmark destination.

The representative workload is the exact tree from the reported 80 MB patch, not an empty checkout. Iteration measurements were 52.7 s, 49.8 s, 25.5 s, 24.0 s, and 23.6 s. The latest two transferred 999,216 framed bytes and verified tree `164e8a770201e7b50ff0f2c09e65e48c6c238931` in fresh destination workspaces. Source preparation is outside the entry timer because the measured operation starts from an already-running source environment. Runtime and repository caches may be warm; a cold runtime install, unavailable origin, large unpublished binaries, lock contention, or poor networking can exceed 30 seconds. The target is measured entry latency, not an unconditional deadline or a reason to skip verification.

`python3 smoke-git-transfer.py <linux|macos|windows>` checks native object transport, binary content, ignored executable files, and capture parity against the existing platform snapshot implementation. It uses only disposable test directories on existing hosts. The Windows CI protocol test additionally exercises the PowerShell command and binary-stdin boundary. During this investigation Linux and macOS guest checks passed; the Windows guest was unreachable over both SSH and Tailscale, so its live SSH path remains a separate validation gap.
...
The configuration-transfer and controller-routing smoke has native Linux, macOS, and Windows command-line evidence at `1ecb753`, plus broker/worker lifecycle coverage using local IPC. Windows passed after repairing missing guest setup through its existing Fleet claim. These checks cover fff search, resource mapping (including stale skill paths), helper execution, and controller-local search/papercut operations; they do not certify desktop GUI behavior or authenticated model prompts on the guest. Session-only extension mode changes and environment-variable overrides are not copied as configuration: persist portable settings in JSON, and inspect provider-mismatch errors or transfer notes rather than assuming a fallback is equivalent.
