...
3. one content-addressed runtime is staged under `~/.cua-pi/runtimes/<digest>`, installs exact npm versions and Git commits with production dependencies but without private copies of Pi peer packages, and is published only after package reconciliation succeeds; generations coexist instead of rewriting shared Pi settings;
