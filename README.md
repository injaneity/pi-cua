...
pi's tui, agent, model, and conversation sessions stay local. each local session has one current execution target: local, linux, windows, or a registered macos host. registered agent tools and user `!` commands execute on that target.
