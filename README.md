...
3. one content-addressed guest bundle is synchronized only when extension files or routed-tool package settings change; package reconciliation must finish before the mutable configuration version is committed;
...
6. the host loads pi's normal remote tool registry, rejects an incomplete required-tool set, then reports one validated protocol and tool manifest;
...
execution placement is stored only as non-context Pi session metadata. restore reads the latest placement across the full session rather than the active branch, so `/tree` and compaction cannot change it. the sandbox name is the durable identity; its saved address is only the last resolved endpoint. the controller keeps one atomic json record per managed sandbox, and every bridge restart renews that endpoint through the normal resume path before opening SSH. no database or background operation queue is involved.
