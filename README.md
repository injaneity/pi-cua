...
this package currently targets macos controllers with `uv` and the Tailscale CLI installed. the controller must be online in a Tailscale network with the `tag:cua-sandbox` ACL tag. its OAuth client must be allowed to create auth keys for that tag, and the tailnet ACL must allow the controller to reach tagged guests, including Tailscale SSH for linux. CUA Fleet pools default to `cua-pi-linux`, `cua-pi-linux-large`, `cua-pi-windows`, and `cua-pi-windows-large`; fleet pool/namespace names are a tenant-wide authorization boundary — if another tenant already owns a default name, pool operations fail with a persistent 403. set `CUA_PI_LINUX_POOL` or `CUA_PI_WINDOWS_POOL` to an unclaimed base name for your tenant; each large pool defaults to that base plus `-large`, with `CUA_PI_LINUX_LARGE_POOL` and `CUA_PI_WINDOWS_LARGE_POOL` available as overrides. store the CUA Fleet and Tailscale OAuth credentials in Keychain:
...
- `/sandbox` changes the current session's execution target. `/sandbox linux large` and `/sandbox windows large` create 16 CPU, 64 GiB sandboxes; omitting the size uses the existing default pools.
